...
artifact fingerprint. When the artifact is missing or stale, the hook returns
`ask` with a warning instead of evaluating stale policy, so Claude Code can use
its normal confirmation flow. Incompatible artifacts still fail closed. Policy
edits, include edits, Claude permission changes, and binary upgrades should be
followed by `cc-bash-guard verify`.
...
- asks with a warning when the verified artifact is missing or stale, and fails
  closed when it is incompatible
...
`cc-bash-guard verify`. When the artifact is missing or stale, the hook returns
`ask` with a warning `systemMessage` and `additionalContext`; it does not
evaluate stale policy. The hook still fails closed when the artifact is
incompatible. The hook does not
regenerate artifacts during hook execution; run `cc-bash-guard verify`
explicitly after policy, include, Claude permission, or binary changes. Stale
