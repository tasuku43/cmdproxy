...
func TestHookHelpDocumentsNoPolicyRewriteAndRTKIntegration(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Missing or stale verified artifacts are regenerated before evaluation",
		"--rtk",
...
func TestHelpPermissionExplainsCurrentSchema(t *testing.T) {
	...
	for _, want := range []string{
		...
		"stderr_to_devnull",
		"stdin_from_devnull",
		"do not allow new commands by themselves",
		"otherwise matching allow rules",
		"stream merges such as 2>&1",
		"dynamic redirect targets",
		"unknown redirects still ask",
